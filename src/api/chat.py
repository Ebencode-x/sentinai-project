...
def _build_system_prompt(incidents: list, stats: dict) -> str:
    ...
    parts = [
        ...
        'Total: ' + str(len(incidents)) + ' | Open: ' + str(len(open_inc)) + ' | Critical: ' + str(len(critical)),
        'Scans: ' + str(stats.get('total_scan_runs', 0)) + ' | Last: ' + str(stats.get('last_scan_at_utc', 'never')),
        '',
        'Incidents:',
        inc_summary,
        '',
