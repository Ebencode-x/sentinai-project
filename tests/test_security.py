...
class TestNoKeyConfigured:
    ...
    def test_request_with_any_header_passes(self, client_no_key):
        assert client_no_key.get("/protected", headers={"X-API-Key": "anything"}).status_code == 200
...
class TestKeyConfigured:
    ...
    def test_correct_key_returns_200(self, client_with_key):
        resp = client_with_key.get("/protected", headers={"X-API-Key": "test-secret-key"})
    ...
    def test_timing_safe_comparison(self, client_with_key):
        ...
        resp_good = client_with_key.get("/protected", headers={"X-API-Key": "test-secret-key"})
        resp_bad = client_with_key.get("/protected", headers={"X-API-Key": "test-secret-ley"})
